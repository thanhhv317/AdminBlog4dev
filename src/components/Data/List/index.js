...
class DataList extends Component {
      ...
      .then(
        (result) => {
          ...
          if (result.status == true) {
            ...
            const index = _.findIndex(items, item => item._id === updateItem._id)
  ...
  render() {
    const { error, isLoaded, items, deleteConfirmation, updateConfirmation, updateItem } = this.state;
    if (error) {
      ...
      return (
        <div className="data-table-area">
                      ...
                      <tbody>
                        {
                          items.map(item => {
                            return <tr key={item._id}>
                              ...
                              <td>
                                <a onClick={() => this.updateConfirmModal(item)}><i className="notika-icon notika-draft" style={{ marginRight: '10px' }}></i></a>
                                <a onClick={() => this.confirmModal(item._id)} ><i className="notika-icon notika-trash" style={{ marginRight: '10px' }}></i></a>
          ...
          <DeleteItem open={deleteConfirmation} onSubmit={this.deleteRow} onClose={this.closeDeleteConfirmation} />
          <UpdateItem open={updateConfirmation} onSubmit={this.updateRow} onChangeUpdate={this.onUpdateDataChange} onClose={this.closeUpdateCOnfirmation} updateItem={{ ...updateItem }} />
        </div>
      );
    }
  }
